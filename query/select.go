...
func buildIterators(ctx context.Context, stmt *influxql.SelectStatement, ic IteratorCreator, opt IteratorOptions) ([]Iterator, error) {
	...
	if len(info.calls) == 0 && len(info.refs) > 0 {
		if scope != nil {
			labels := metrics.Labels("statement", stmt.String())
			scope = scope.NewScope("auxiliary_iterators", metrics.ScopeLabels(labels))
	...
	if scope != nil {
		labels := metrics.Labels("statement", stmt.String())
		scope = scope.NewScope("field_iterators", metrics.ScopeLabels(labels))
		ctx = metrics.NewContextWithScope(ctx, scope)
	}
	...
	if err := func() error {
		...
		for i, f := range fields {
			...
			if scope := metrics.ScopeFromContext(ctx); scope != nil {
				labels := metrics.Labels("expr", f.Expr.String())
				scope = scope.NewScope("expression", metrics.ScopeLabels(labels))
				ctx = metrics.NewContextWithScope(ctx, scope)
			}

			itr, err := buildExprIterator(ctx, expr, ic, sources, opt, selector, writeMode)
			if err != nil {
				return err
			} else if itr == nil {
