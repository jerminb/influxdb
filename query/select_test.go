...
func TestSelect(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			itrs, _, err := query.Select(context.TODO(), MustParseSelectStatement(tt.q), &shardMapper, query.SelectOptions{})
			if err != nil {
				if tt.err == "" {
					t.Fatal(err)
...
func TestSelect_Raw(t *testing.T) {
	...
	itrs, _, err := query.Select(context.TODO(), stmt, &shardMapper, query.SelectOptions{})
	if err != nil {
