...
func TestShard_WritePoints_FieldConflictConcurrentQuery(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 500; i++ {
			...
			iter, err := sh.CreateIterator(context.TODO(), "cpu", query.IteratorOptions{
...
func TestShard_CreateIterator_Ascending(t *testing.T) {
	...
	test := func(index string) {
		...
		// ErrEngineClosed.
		_, got := sh.CreateIterator(context.TODO(), "cpu", query.IteratorOptions{})
...
`)
		...
		// Create iterator.
		var err error
		itr, err = sh.CreateIterator(context.TODO(), "cpu", query.IteratorOptions{
			Expr:       influxql.MustParseExpr(`value`),
...
func TestShard_Disabled_WriteQuery(t *testing.T) {
	...
	test := func(index string) {
		...
		if _, err = sh.CreateIterator(context.TODO(), "cpu", query.IteratorOptions{}); err != nil {
