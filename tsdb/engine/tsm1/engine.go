...
var (
	...
	numberOfRefCursorsCounter  = metrics.MustRegisterCounter("ref_cursors", metrics.WithGroup(tsmGroup))
	numberOfAuxCursorsCounter  = metrics.MustRegisterCounter("aux_cursors", metrics.WithGroup(tsmGroup))
	numberOfCondCursorsCounter = metrics.MustRegisterCounter("cond_cursors", metrics.WithGroup(tsmGroup))
	processingTimer            = metrics.MustRegisterTimer("processing_time", metrics.WithGroup(tsmGroup))
)
...
func (e *Engine) CreateIterator(ctx context.Context, measurement string, opt query.IteratorOptions) (query.Iterator, error) {
	if scope := metrics.ScopeFromContext(ctx); scope != nil {
		labels := metrics.Labels("shard_id", strconv.Itoa(int(e.id)), "measurement", measurement)
		if opt.Condition != nil {
			labels.Merge(metrics.Labels("cond", opt.Condition.String()))
		}
		scope = scope.NewScope("create_iterator", metrics.ScopeLabels(labels))
		ctx = metrics.NewContextWithScope(ctx, scope)
		group := scope.NewGroup(tsmGroup)
		ctx = metrics.NewContextWithGroup(ctx, group)
		start := time.Now()
		defer group.GetTimer(processingTimer).UpdateSince(start)
	}
